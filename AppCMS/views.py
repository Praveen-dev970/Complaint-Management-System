...
def logout_view(request):
    ...
    return redirect('signin')  # Redirect to the home page or any other page you prefer
...
def change_password_g(request):
    ...
    return render(request, 'change_password_g.html', {
        'form': form
    })
    return render(request,"change_password_g.html")
...
def list(request):
    ...
    return render(request,'Complaints.html',args)
@login_required
def slist(request):
    result=Complaint.objects.filter(user=request.user).exclude(Q(status='3') | Q(status='2'))
    #c=Complaint.objects.all()
    args={'result':result}
    return render(request,'solvedcomplaint.html',args)
...
def allcomplaints(request):
        ...
        return render(request,'allcomplaints.html',args)
...
def solved(request):
        ...
        args={'c':c,'forms':forms,'comp':comp}
        return render(request,'solved.html',args)
